...
(defrecord State [loc minutes open-valves directory dists]
  dijkstra/IState
  (state-key [_this] loc)

  (cost [_this]
        (+ (* minutes (:pressure (directory loc)))
           #_
           (* 10 minutes (- (count directory) (count open-valves)))))

  (next-states [_this]
    ;; return distance (minutes burned) to each still-open valve, including the
    ;; minute burned to open that valve.
    (let [nexts (for [v (->> directory (keep (fn [[k v]] (when (and (not= k loc) (neg? (:pressure v))) k))))
                      :let [burned (+ 1 (dists [loc v]))]
                      #_#_:when (< burned minutes)]
                  (->State v (- minutes burned) (conj open-valves v) directory dists))]
      (if (seq nexts) nexts [(->State loc 0 open-valves directory dists)])))
...
(comment
  (let [directory (parse (slurp "input/2022/16-pressure-sample.txt"))
    ...
    (->> (->State "AA" 30 open-valves directory dists)
         dijkstra/lowest-cost
         :node
         dijkstra/path
         (map #(update % ::dijkstra/state dissoc :directory :dists :open-valves))
         #_dijkstra/path))
